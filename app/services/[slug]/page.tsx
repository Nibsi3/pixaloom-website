...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{
  ...
  return pageMetadata({title:`${service.shortName} South Africa`,description:service.description,path:`/services/${slug}`});
}

export default async function ServicePage({params}:{params:Promise<{slug:string}>}){
  const {slug}=await params; const service=getService(slug); if(!service)notFound();
  const schema={'@context':'https://schema.org','@graph':[{'@type':'Service','@id':absoluteUrl(`/services/${slug}#service`),name:service.name,description:service.description,provider:{'@id':`${site.url}/#organization`},areaServed:{'@type':'Country',name:'South Africa'},url:absoluteUrl(`/services/${slug}`)},{'@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'Home',item:site.url},{'@type':'ListItem',position:2,name:'Services',item:absoluteUrl('/services')},{'@type':'ListItem',position:3,name:service.name,item:absoluteUrl(`/services/${slug}`)}]}]};
