...
export function ServiceProof({ slug }: { slug: string }) {
  ...
  return <>
    <section className="content-section"><div className="site-container content-grid"><div><p className="eyebrow">Scope and decisions</p><h2>{evidence.title}</h2></div><div className="body-copy">{evidence.paragraphs.map(paragraph => <p key={paragraph}>{paragraph}</p>)}<div className="minimal-inline-links"><Link className="text-link" href={`/blog/${evidence.guide}`}>Read the planning guide</Link>{evidence.related?.map(link => <Link className="text-link" href={link.href} key={link.href}>{link.label}</Link>)}</div></div></div></section>
